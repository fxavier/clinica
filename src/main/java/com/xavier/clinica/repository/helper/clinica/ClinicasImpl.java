...
public class ClinicasImpl implements ClinicasQueries{
	...
	public Page<Clinica> filtrar(ClinicaFilter filtro, Pageable pageable) {
		...
		Criteria criteria = manager.unwrap(Session.class).createCriteria(Clinica.class);
	...
	private Long total(ClinicaFilter filtro) {
		@SuppressWarnings("deprecation")
		Criteria criteria = manager.unwrap(Session.class).createCriteria(Clinica.class);
		adicionarFiltro(filtro, criteria);
	...
	private void adicionarFiltro(ClinicaFilter filtro, Criteria criteria) {
		if(filtro != null) {
